...
export default function Nav() {
  ...
  return (
    ...
    // 20px background blur, padding 4px 0. Width increased 20% (840 -> 1008px) and height
    // increased a bit (4px -> 8px vertical padding) per explicit request.
    <header className="fixed top-6 left-1/2 z-50 w-[92%] max-w-[1008px] -translate-x-1/2">
      <div className="flex items-center gap-[49px] rounded-[20px] border border-borderColor bg-[rgba(1,20,24,0.3)] px-[30px] py-2 backdrop-blur-[20px]">
        ...
        <div className="flex items-center gap-2.5">
          {/* Real "Nav / Nav Link" component (framer.agent.getNode("qMV5jssFV")): container
              gap 0, each link's own padding is 6px 10px (its real click-target size), text
              uses the real "Body 16" style preset (PT Mono, 16px, -0.02em letter-spacing). */}
          <nav className="hidden items-center gap-0 md:flex">
            {links.map((l) => (
              <Link
                key={l.text}
                href={l.href}
                className="px-[10px] py-[6px] font-mono text-base tracking-[-0.02em] text-white/65 hover:text-white transition-colors"
              >
                {l.text}
              </Link>
            ))}
        ...
        <div className="mt-2 flex flex-col gap-4 rounded-2xl border border-borderColor bg-black/90 p-4 md:hidden">
          {links.map((l) => (
            <Link
              key={l.text}
              href={l.href}
              className="font-mono text-base tracking-[-0.02em] text-white/80"
